...
namespace HK_SDK_DEMO_回调采图
{
    public partial class GrabImage_Callback : Form
    {
        ...
        private void ImageCallBackFunc(IntPtr pData, ref MV_FRAME_OUT_INFO_EX pFrameInfo, IntPtr pUser)
        {
            ...
            //回调显示图像
            MV_DISPLAY_FRAME_INFO displayInfo = new MV_DISPLAY_FRAME_INFO();
            displayInfo.pData = pData;
            displayInfo.hWnd = this.pictureBox1.Handle;
            displayInfo.nHeight = pFrameInfo.nHeight;
            displayInfo.nWidth = pFrameInfo.nWidth;
            displayInfo.enPixelType = pFrameInfo.enPixelType;
            displayInfo.nDataLen = pFrameInfo.nFrameLen;
            myCamera.MV_CC_DisplayOneFrame_NET(ref displayInfo);
        }
